function []= MakeTheCategoricalLinePlot(Response, avg_Tmp,  err_Tmp, category, n, vertline)
global newdir

% Break imaging traces into two groups, depending on the
% categorical grouping variable (logical array).
Response_group1 = Response(:,category); 
Response_group2 = Response(:,~category);
...
% plot([1:size(Response_group1,1)],Response_group1, 'LineWidth', 1);
% plot([1:size(Response_group1,1)],median(Response_group1,2, 'omitnan'),'LineWidth', 2, 'Color', 'k');

 plot([1:size(Response_group2,1)],Response_group2, 'LineWidth', 1);
 plot([1:size(Response_group2,1)],median(Response_group2,2, 'omitnan'),'LineWidth', 2, 'Color', 'k');
...
title(currentFigure.Children(end), strcat(n,'_ Avg Response'),'Interpreter','none');
